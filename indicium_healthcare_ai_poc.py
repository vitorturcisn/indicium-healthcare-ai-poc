...
13. VALIDAÇÃO
O relatório final será validado automaticamente.
Se houver erro metodológico, corrija antes de finalizar.
"""
...
def construir_grafo(
    ...
    Caso o validador encontre problemas na primeira tentativa,
    o agente recebe uma solicitação de correção e gera uma nova versão.
    """
    ...
    def validator_node(
        ...
        if (
            not resultado["ok"]
            and tentativas < MAX_VALIDATION_ATTEMPTS
        ):
            problemas = "\n".join(
                f"- {problema}"
            ...
            mensagem_correcao = HumanMessage(
                content=(
                    "VALIDAÇÃO AUTOMÁTICA DO RELATÓRIO:\n"
                    f"{problemas}\n\n"
                    "Gere novamente o relatório corrigindo "
                    "todos os problemas apontados. "
                    "Mantenha os números devolvidos pelas tools. "
    ...
    def route_after_validator(
        ...
        if tentativas >= MAX_VALIDATION_ATTEMPTS:
            return END
